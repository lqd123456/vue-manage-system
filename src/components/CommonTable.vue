  ...
  <div class="common-table">
    <el-table v-loading="config.loading" :data="tableData" height="90%" stripe>
      <el-table-column label="序号" width="85">
        <template slot-scope="scope"> <!-- 通过插槽的形式暴露组件内部的参数 -->
          <span style="margin-left: 10px">{{ (config.page - 1) * 20 + scope.$index + 1 }}</span>
        </template>
      </el-table-column>
      <el-table-column v-for="item in tableLabel" :key="item.prop" show-overflow-tooltip :label="item.label" :width="item.width ? item.width : 125">
